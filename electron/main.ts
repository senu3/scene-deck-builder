...
function computeGrayHistogram(frame: Buffer, bins: number): number[] {
  const hist = new Array<number>(bins).fill(0);
  const scale = bins / 256;
  for (let i = 0; i < frame.length; i++) {
    const value = frame[i] ?? 0;
    const index = Math.min(bins - 1, Math.floor(value * scale));
    hist[index] += 1;
  }
  return hist;
}

function histogramDistanceL1(a: number[], b: number[], pixelCount: number): number {
...
function analyzeVideoHistogramViaFfmpeg(
  ffmpegBinary: string,
  options: AnalyzeVideoHistogramOptions
): Promise<AnalyzeVideoHistogramResult> {
  const sampleFps = Number.isFinite(options.sampleFps) ? Math.max(1, Math.min(12, Math.floor(options.sampleFps as number))) : 4;
  const width = Number.isFinite(options.width) ? Math.max(16, Math.min(256, Math.floor(options.width as number))) : 64;
  const height = Number.isFinite(options.height) ? Math.max(16, Math.min(256, Math.floor(options.height as number))) : 36;
  const startSec = Number.isFinite(options.startSec) ? Math.max(0, options.startSec as number) : 0;
  const endSec = Number.isFinite(options.endSec) ? Math.max(0, options.endSec as number) : 0;
  const hasRange = endSec > startSec + 0.02;
  ...
  const args = [
    ...
    ...(hasRange ? ['-t', (endSec - startSec).toFixed(3)] : []),
    '-an',
    '-vf', `fps=${sampleFps},scale=${width}:${height}:flags=fast_bilinear,format=gray`,
    '-pix_fmt', 'gray',
    '-f', 'rawvideo',
    'pipe:1',
  ];

  return ffmpegLightQueue.enqueue(() => new Promise<AnalyzeVideoHistogramResult>((resolve) => {
    const proc = spawn(ffmpegBinary, args);
    const stderrRing = createStderrRing();
    const scores: number[] = [];
    ...
    const finalize = (result: AnalyzeVideoHistogramResult) => {
      resolve(result);
    };
    ...
    proc.stdout.on('data', (chunk: Buffer) => {
      ...
      while (pending.length >= frameSize) {
        ...
        const current = computeGrayHistogram(frame, bins);
        const score = previousHistogram
          ? histogramDistanceL1(current, previousHistogram, frameSize)
          : 0;
        scores.push(score);
        previousHistogram = current;
      }
    });

    proc.stderr.on('data', (chunk: Buffer) => {
      appendStderr(stderrRing, chunk, ffmpegLimits.stderrMaxBytes);
    });
    ...
    proc.on('close', (code: number | null) => {
      if (code !== 0) {
        finalize({
          success: false,
          error: getStderrText(stderrRing) || `ffmpeg exited with code ${code}`,
        });
        return;
      }

      if (scores.length === 0) {
        finalize({ success: false, error: 'No frames sampled from video.' });
        return;
      }
    ...
    proc.on('error', (error: Error) => {
      finalize({ success: false, error: `Failed to start ffmpeg: ${error.message}` });
    });
  }));
}
...
ipcMain.handle('analyze-video-histogram', async (_, options: AnalyzeVideoHistogramOptions): Promise<AnalyzeVideoHistogramResult> => {
  const ffmpegBinary = ffmpegPath as string | null;
  if (!ffmpegBinary) {
    return { success: false, error: 'ffmpeg not found' };
  }
  ...
  try {
    return await analyzeVideoHistogramViaFfmpeg(ffmpegBinary, options);
  } catch (error) {
    console.error('[AutoClip] analyze-video-histogram failed:', error);
    return { success: false, error: 'analyze-video-histogram failed' };
  }
});
...
ipcMain.handle('export-sequence', async (_, options: ExportSequenceOptions): Promise<ExportSequenceResult> => {
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.lipSync) {
        const lipSyncValidationError = validateLipSyncExportPayload(item.lipSync);
        if (lipSyncValidationError) {
          throw new Error(`[export] Invalid lipSync payload for segment ${i}: ${lipSyncValidationError}`);
        }
