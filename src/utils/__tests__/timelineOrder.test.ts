...
const buildScene = (id: string, cutDefs: Array<{ id: string; order: number }>): Scene => ({
  id,
  name: id,
  notes: [],
...
describe('timeline order helpers', () => {
  it('sorts scenes and cuts by timeline order', () => {
    const scenes: Scene[] = [
      buildScene('scene-b', [{ id: 'cut-b2', order: 1 }, { id: 'cut-b1', order: 0 }]),
      buildScene('scene-a', [{ id: 'cut-a2', order: 1 }, { id: 'cut-a1', order: 0 }]),
    ];
    const sceneOrder = ['scene-a', 'scene-b'];

    const ordered = getScenesAndCutsInTimelineOrder(scenes, sceneOrder);
  ...
  it('normalizes selected cut ids to timeline order regardless of selection order', () => {
    const scenes: Scene[] = [
      buildScene('scene-b', [{ id: 'cut-b1', order: 0 }]),
      buildScene('scene-a', [{ id: 'cut-a2', order: 1 }, { id: 'cut-a1', order: 0 }]),
    ];
    const sceneOrder = ['scene-a', 'scene-b'];

    const orderedIds = getCutIdsInTimelineOrder(scenes, ['cut-b1', 'cut-a2', 'cut-a1'], sceneOrder);
