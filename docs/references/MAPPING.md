...
| アセットパネル | `Asset` / `AssetIndexEntry`。 | `loadAssetIndex` / `getFolderContents` / `metadataStore` 集計（LipSync生成アセット除外、ownerバッジ付与）+ `assetActions` 経由の Asset 操作（Finalize/Reverse/Extract/Delete）。 | `AssetPanel`, `actions/assetActions.ts` |
...
| エクスポート実行境界 | `ExportSequenceOptions` / `ExportSequenceResult`（IPC payload, `subtitleStyle` 含む）。 | `window.electronAPI.exportSequence`。 | `App`、`PreviewModal` |
