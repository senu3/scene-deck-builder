...
| アセットパネル | `Asset` / `AssetIndexEntry`。 | `readAssetIndex` / `getFolderContents` / `metadataStore` 集計（LipSync生成アセット除外、ownerバッジ付与）+ `assetActions` 経由の Asset 操作（Finalize/Reverse/Extract/Delete）+ 外部DnD（`startAssetFileDrag`）。 | `AssetPanel`, `features/asset/actions.ts`, `electron/preload.ts`, `electron/main.ts` |
