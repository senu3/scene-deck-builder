...
- root の barrel export は `stable` を優先し、`candidate` を追加する場合は採用先または移行対象を明確にする。
...
- close / more / menu trigger のような icon-only button が増える場合は、feature ごとの個別 CSS を増やす前に `IconButton` primitive を追加する。
...
- `Input`, `Select`, `Checkbox`, `RadioGroup`, `Toggle`, `Tabs`
