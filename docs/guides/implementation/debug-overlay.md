...
### Diagnostic表示
- SequencePlan の debug/diagnostic は `SequencePlan.warnings` の構造化データを表示対象とする。
- effect activity は `issued / start / success / failure` と `effectType / orderingKey / commandType` を表示対象とする。
- debug用に Export helper / Preview hook へ implicit logging を再導入しない。
- HUDは diagnostic の表示のみを行い、Plan解決ロジック自体は持たない。
...
- `debug/modules/effectActivityDebug.ts`
...
- in-memory activity log の購読もOverlay module内に閉じる
