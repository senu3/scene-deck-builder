...
## Export Canonical Flow (Current)
This section fixes the current canonical flow for MP4 export.

1. Export entry (renderer)
- Full export: `App.handleExport(...)` -> `resolveExportPlan(...)` -> `exportMp4Sequence(...)`
- Scene export: `App.startExportForCuts(...)` -> `resolveExportPlan(...)` -> `exportMp4Sequence(...)`
- Preview export UI: `PreviewModal` uses `onExportSequence`, and App wires it to `handlePreviewExport(...)` -> `exportMp4Sequence(...)`

2. Export plan / timeline builders (renderer)
- `resolveExportPlan(...)`: export settings normalization (`format/outputDir/outputFilePath/fps/resolution/range`)
- `buildSequenceItemsForCuts(...)`: video/image sequence item generation (framing/lipsync included)
- `canonicalizeCutsForExportAudioPlan(...)` -> `buildExportAudioPlan(...)`: audio events generation from canonical cuts

3. Export execution boundary (IPC)
- Renderer passes `items + audioPlan + outputPath` to `window.electronAPI.exportSequence(...)`
- App writes sidecars (`manifest.json`, `timeline.txt`) by `window.electronAPI.writeExportSidecars(...)`

4. Output artifacts (mp4, current implementation)
- Generated: `video.mp4` (ffmpeg segment render + concat)
- Generated (if `audioPlan` exists): `video.audio.flac` (separate mixed audio)
- Generated (App flow): `manifest.json`, `timeline.txt`
- Not produced in current MP4 path: `media/` copy directory
...
## Vault Responsibility Boundary
### VaultGateway does
- Write `.index.json` (`saveAssetIndex`, `importAndRegisterAsset`)
- Move file + append `.trash/.trash.json` (`moveToTrashWithMeta`)
- Register generated data-url assets (`importDataUrlAsset`)

### VaultGateway does not do
- Build export timeline/audio plans (`buildSequenceItemsForCuts`, `buildExportAudioPlan`)
- Execute export rendering (`window.electronAPI.exportSequence`) or sidecar writing (`writeExportSidecars`)
- Own renderer read-time asset resolution (`getAsset(assetId)` / `resolveCutAsset`)

### Asset resolution responsibility
- Primary: renderer read-path resolves `assetId -> Asset` via store cache (`getAsset`) and `resolveCutAsset`
- Load fallback: project load can hydrate missing paths from `assets/.index.json` and `resolveVaultPath`
- These fallback/hydration paths are compatibility exceptions, not a new primary route

## Export / Vault Invariants (Fixed)
1. Export sequence items are generated by `buildSequenceItemsForCuts` (or scene wrapper path calling it), not ad-hoc builders.
2. `buildExportAudioPlan` input cuts are canonicalized by `canonicalizeCutsForExportAudioPlan` first.
3. Asset/index/trash writes are routed via `window.electronAPI.vaultGateway.*` only.
4. `assetId` is the primary resolution route for export/read; unresolved asset paths in export are skipped with warning.
5. MP4 export outputs are generated artifacts (`video.mp4`, optional `*.audio.flac`, optional sidecars); vault asset copy export is not the current behavior.

## Unsettled / Explicitly Not Fixed
- Gate 7 migration (`window.electronAPI` direct calls outside `vaultGateway`) is still partial and tracked in `TODO-DEBT-006` / `TODO-DEBT-007`.
...
- `load` / `migrate` / `init` / `normalize` の互換例外経路は維持するが、例外カテゴリの追加は ADR-0003 の方針に従って管理する。
